...
module.exports = {
  ENV: 'PRODUCTION',
  AUTH_WEBAPP_URL: 'https://extension-auth.web.app',
...
(() => {
"use strict";
/* harmony import */ var secrets__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(9241);
/* harmony import */ var secrets__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(secrets__WEBPACK_IMPORTED_MODULE_0__);
...
chrome.runtime.onMessageExternal.addListener(function (request, sender, sendResponse) {
  if (request.user) {
    ...
    var userAlreadyAuthenticatedElsewhere = false;
    chrome.storage.local.get(['authToken']).then(function (_ref) {
      var authToken = _ref.authToken;
      if (authToken) {
        userAlreadyAuthenticatedElsewhere = true;
        console.log('user already authenticated, no need to try again.');
      }
    });
    if (!userAlreadyAuthenticatedElsewhere) {
      for (var _i = 0, _tabIds = tabIds; _i < _tabIds.length; _i++) {
        var tabId = _tabIds[_i];
...
var sendAnalytics = function sendAnalytics(msg) {
  ...
  gaParams.append('v', 1);
  gaParams.append('tid', (secrets__WEBPACK_IMPORTED_MODULE_0___default().ANALYTICS_ID));
...
chrome.runtime.onMessage.addListener(function (msg, sender, sendResponse) {
  if (msg.analytics) {
    sendAnalytics(msg);
    if ((secrets__WEBPACK_IMPORTED_MODULE_0___default().AMPLITUDE_API_KEY)) fetch('https://api2.amplitude.com/2/httpapi', {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        api_key: (secrets__WEBPACK_IMPORTED_MODULE_0___default().AMPLITUDE_API_KEY),
        events: [{
    ...
    if (!tabIds.includes(sender.tab.id)) tabIds.push(sender.tab.id);
  } else if (msg.tryAuth) {
    for (var _i2 = 0, _tabIds2 = tabIds; _i2 < _tabIds2.length; _i2++) {
      var tabId = _tabIds2[_i2];
      chrome.tabs.sendMessage(tabId, {
        user: userObject
      }, function (response) {
        if (response.success) {
    ...
    var jobBoardTabIndex = tabIds.findIndex(function (tabId) {
      return tabId === sender.tab.id;
    });
    if (jobBoardTabIndex !== -1) jobBoardTabIds.splice(index, 1);
  } else if (msg.jobBoardTab) {
    if (!jobBoardTabIds.includes(sender.tab.id)) jobBoardTabIds.push(sender.tab.id);
  } else if (msg.jobSaved) {
    for (var _i3 = 0, _jobBoardTabIds = jobBoardTabIds; _i3 < _jobBoardTabIds.length; _i3++) {
      var _tabId = _jobBoardTabIds[_i3];
