...
func scrapeStocks(ticker string, c *colly.Collector, stocks *[]Stock) {
	...
	cc.OnHTML("html", func(e *colly.HTMLElement) {
		...
		file, err := os.Create("response.log")
		if err != nil {
	...
	cc.OnHTML("quote-price", func(e *colly.HTMLElement) {
		stock := Stock{
			Company: strings.TrimSpace(e.ChildText("data-symbol")),
			Price:   strings.TrimSpace(e.ChildText("fin-streamer[data-field='livePrice']")),
			Change:  strings.TrimSpace(e.ChildText("fin-streamer[data-field='priceChange']")),
		}
